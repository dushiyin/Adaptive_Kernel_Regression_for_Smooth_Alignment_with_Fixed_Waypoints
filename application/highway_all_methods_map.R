...
anw_fit_at_X_star <- function(Ym_star) {
  vapply(
    X_base,
    function(x0) ANW_1d_point(
      x0 = x0, X = X_base, Y = Ym_star, Xc = Xc, Yc = Yc_star,
      h = h_ANW_opt_1d, Lambda = Lambda_opt_1d
    ),
    numeric(1)
  )
...
ds_anw_predict_star <- function(M = 1) {
  ...
  vapply(
    xg,
    function(x0) ANW_1d_point(
      x0 = x0, X = X_base, Y = Ym, Xc = Xc, Yc = Yc_star,
      h = h_ANW_opt_1d, Lambda = Lambda_opt_1d
    ),
    numeric(1)
  )
...
pdf("figs/fig_highway11_rotated.pdf", width = 9, height = 6)
print(p_rot)
dev.off()

curves_real <- curves_rot %>%
  transmute(lon = lon, lat = lat, method = method)

x_rng <- range(c(coords_all$lon, cons_hwy$lon, curves_real$lon), na.rm = TRUE)
y_rng <- range(c(coords_all$lat, cons_hwy$lat, curves_real$lat), na.rm = TRUE)

pad_x <- diff(x_rng) * 0.08
pad_y <- diff(y_rng) * 0.08

x_lim <- x_rng + c(-pad_x, pad_x)
y_lim <- y_rng + c(-pad_y, pad_y)
...
p_map <- ggplot() +
  ...
  labs(
    title = "Highway 11: All Methods on Map (CartoDB)",
    subtitle = sprintf("NW: h=%.4f | ANW/DS-ANW: h=%.4f, Lambda=%.1f | alpha=%.1f",
                       h_nw_opt_1d, h_ANW_opt_1d, Lambda_opt_1d, alpha),
    x = "Longitude", y = "Latitude",
    color = "Method", linetype = "Method", linewidth = "Method",
    caption = "Map tiles by CartoDB"
  ) +
  scale_color_manual(values = c(
    "Baseline"      = "black",
    "NW"            = "green4",
...
pdf("figs/fig_highway11_map.pdf", width = 8.5, height = 10)
print(p_map)
dev.off()

cat("Saved:\n",
    " - figs/fig_highway11_rotated.pdf\n",
    " - figs/fig_highway11_map.pdf\n")
